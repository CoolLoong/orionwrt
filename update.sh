...
reset_feeds_conf() {
    if [[ "$SKIP_FEEDS_UPDATE" == "true" ]]; then
        echo "跳过 feeds 重置和更新操作..."
        return 0
    fi
    echo "执行 feeds 重置和更新操作..."
...
install_feeds() {
    if [[ "$SKIP_FEEDS_UPDATE" == "true" ]]; then
        echo "跳过完整的 feeds 安装操作..."
        return 0
    fi

    echo "执行完整的 feeds 安装操作..."
...
update_orion() {
    local repo_url="https://github.com/CoolLoong/luci-theme-orion.git"
    local dst_theme_path="$BUILD_DIR/feeds/luci/themes/luci-theme-orion"
    ...
    rm -rf "$temp_dir/.git"
    mv "$temp_dir" "$dst_theme_path"

    echo "luci-theme-orion 更新完成"
    echo "Orion 更新完毕。"
}
...
install_mihomo_for_openclash() {
    ...
    if command -v wget >/dev/null 2>&1; then
        ...
        curl -L --retry 3 --max-time 30 -o "$mihomo_gz" "$mihomo_url"
    else
